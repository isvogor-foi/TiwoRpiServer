...
int read_jpeg_file(char *filename, unsigned char ** buffer)
{
  ...
  buffer[0] = (unsigned char*) malloc(cinfo.output_width*cinfo.output_height*cinfo.num_components );
...
// this is very suspicious -> try to read and send ?
int read_jpeg_mem(unsigned char *in_image, int image_size, unsigned char ** buffer)
{
  ...
  JSAMPROW row_pointer[1];
  // ili ovo...
  printf("Size of input libjpeg: %d", image_size);
  // --- mozda je premala velicina?
  FILE * infile = fmemopen(in_image, image_size, "rb");
  //FILE * infile = fmemopen(in_image, sizeof(in_image), "rb"); <- Fucking wrong code pasted form Internet, return size of pointer!!!
